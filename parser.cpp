...
private:
    ...
    void parseStatement() {
        if (tokens[pos].type == T_INT) {
            parseDeclaration(); //check for integer declaration
        } else if (tokens[pos].type == T_ID) {
            parseAssignment(); //check for identifier assignment
        } else if (tokens[pos].type == T_IF) {
            parseIfStatement(); //check for if statement
        } else if (tokens[pos].type == T_RETURN) {
            parseReturnStatement(); //check for return statement
        } else if (tokens[pos].type == T_LBRACE) {
            parseBlock(); //to parse entire block inside a bracket
        } else {
